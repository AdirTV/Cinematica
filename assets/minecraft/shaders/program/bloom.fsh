...
out vec4 fragColor;  // Added output variable for GLSL 150
...
float estimate_luminance() {
    ...
    for (int ii = 0; ii < TAPS_X; ++ii) {
        ...
        for (int jj = 0; jj < TAPS_Y; ++jj) {
            ...
            accum += luminance(texture(DiffuseSampler, vec2(xx, yy)).rgb);  // Replaced 'texture2D' with 'texture'
        }
    }
...
vec3 bloom() {
    ...
    for (int ii = 0; ii < TAPS; ++ii) {
        for (int jj = 0; jj < DISTANCE; ++jj) {
            color += texture(DiffuseSampler, texCoord + (tapOffset * float(jj + 1)));  // Replaced 'texture2D' with 'texture'
        }
...
void main() {
    vec3 color = texture(DiffuseSampler, texCoord).rgb;  // Replaced 'texture2D' with 'texture'
    ...
    fragColor = vec4(color + bloom() * hdr, 1.0);  // Replaced 'gl_FragColor' with 'fragColor'
}